...
namespace LIV.Shared.Pathfinder
{
    ...
    public class PObject : IDisposable
    {
        ...
        public PObject(string key)
        {
            ...
            SDKBridgeVolcap.GetValue(key, out _pathfinderPointer, (int)PathfinderType.Pointer);
        }
    ...
    public class SDKBridgeVolcap
    {
        ...
        [DllImport("BONEWORKS_Data/Plugins/LIV_Bridge.dll", EntryPoint = "LivSDKSet")]
        ...
        [DllImport("BONEWORKS_Data/Plugins/LIV_Bridge.dll", EntryPoint = "LivSDKGet")]
        [SuppressUnmanagedCodeSecurity]
        ...
        [DllImport("BONEWORKS_Data/Plugins/LIV_Bridge.dll", EntryPoint = "NJSet")]
        [SuppressUnmanagedCodeSecurity]
        public static extern int njset(IntPtr nj, string path, IntPtr value, int valuelength, int valuetype);

        [DllImport("BONEWORKS_Data/Plugins/LIV_Bridge.dll", EntryPoint = "NJGet")]
        [SuppressUnmanagedCodeSecurity]
        public static extern int njget(IntPtr nj, string path, IntPtr value, int valuelength, int valuetype);

        [DllImport("BONEWORKS_Data/Plugins/LIV_Bridge.dll", CallingConvention = CallingConvention.Cdecl)]
        [SuppressUnmanagedCodeSecurity]
        ...
        {
            var pfCheck = SDKBridgeVolcap.AsPFPointer(pathCheck);
        ...
        {
            var pf = SDKBridgeVolcap.CreatePathfinderObject();
            SDKBridgeVolcap.SetPFObject(rootNode, pf);
            SDKBridgeVolcap.FreePFPointer(pf);
        }
    ...
    public class NJCursor
    {
        ...
        public T Value<T>()
        {
            switch (Type.GetTypeCode(typeof(T)))
            {
                case TypeCode.String:
                    var result = "";
                    SDKBridgeVolcap.GetString(Key, ref result);
                    return (T)Convert.ChangeType(result, typeof(T));
                case TypeCode.Boolean:
                    T boolVal;
                    SDKBridgeVolcap.GetValue<T>(Key, out boolVal, (int)PathfinderType.Boolean);
                    return boolVal;
                default:
                    T value;
                    SDKBridgeVolcap.GetValue<T>(Key, out value, (int)Type.GetTypeCode(typeof(T)));
                    return value;
            }
        }
        ...
        {
            var v = string.Empty;
            return SDKBridgeVolcap.GetString(Key, ref v);
        }
    ...
    public class LIVSDK
    {
        ...
        {
            int flagValue = 1;
            SDKBridgeVolcap.SetValue($"LIV.features.{(feature)}", ref flagValue, 9);
        }
